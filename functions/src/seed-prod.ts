...
async function seedProd() {
  ...
  // 1. Find the real user by email
  let userRecord;
  try {
    userRecord = await admin.auth().getUserByEmail("anknarsi@gmail.com");
    console.log(`  ✓ Found user: ${userRecord.uid} (${userRecord.email})`);
  } catch {
    console.error("User anknarsi@gmail.com not found in Firebase Auth. Please sign in first via the app.");
    process.exit(1);
  }
  ...
  // 3. Create user document linking to real Firebase Auth user
  ...
  // 4. Create borrowers
  const borrowers = [
    { id: "demo-borrower", firstName: "John", lastName: "Smith", email: "john.smith@example.com", phone: "+12145551234", loanNumber: "LN-2024-001" },
    { id: "borrower-maria", firstName: "Maria", lastName: "Garcia", email: "maria.garcia@example.com", phone: "+12145552345", loanNumber: "LN-2024-002" },
    { id: "borrower-james", firstName: "James", lastName: "Wilson", email: "james.wilson@example.com", phone: "+12145553456", loanNumber: "LN-2024-003" },
    { id: "borrower-sarah", firstName: "Sarah", lastName: "Chen", email: "sarah.chen@example.com", phone: "+12145554567", loanNumber: "LN-2024-004" },
  ];

  for (const b of borrowers) {
    await db.collection("borrowers").doc(b.id).set({
      id: b.id, organizationId: "demo-org", firstName: b.firstName, lastName: b.lastName,
      email: b.email, phone: b.phone, loanNumber: b.loanNumber,
      createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
    });
    console.log(`  ✓ Borrower: ${b.firstName} ${b.lastName}`);
  }
  ...
  for (const v of vehicles) {
    await db.collection("vehicles").doc(v.id).set({
      id: v.id, borrowerId: v.borrowerId, organizationId: "demo-org",
  ...
  await db.collection("policies").doc("demo-policy").set({
    id: "demo-policy", vehicleId: "demo-vehicle", borrowerId: "demo-borrower", organizationId: "demo-org",
    status: "ACTIVE", policyNumber: "POL-ABC-123456", policyTypes: ["AUTO"],
    ...
    vehicleRemovedFromPolicy: false, complianceIssues: [], dashboardStatus: "GREEN",
    lastVerifiedAt: Timestamp.now(), createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
  });
  console.log("  ✓ Policy: John Smith (GREEN — fully compliant)");

  // YELLOW: Maria Garcia — expiring soon
  await db.collection("policies").doc("policy-maria").set({
    id: "policy-maria", vehicleId: "vehicle-maria", borrowerId: "borrower-maria", organizationId: "demo-org",
    status: "ACTIVE", policyNumber: "POL-XYZ-789012", policyTypes: ["AUTO"],
    ...
    vehicleRemovedFromPolicy: false, complianceIssues: ["EXPIRING_SOON"], dashboardStatus: "YELLOW",
    lastVerifiedAt: Timestamp.now(), createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
  });
  console.log("  ✓ Policy: Maria Garcia (YELLOW — expiring soon)");

  // RED: James Wilson — cancelled, no lienholder
  await db.collection("policies").doc("policy-james").set({
    id: "policy-james", vehicleId: "vehicle-james", borrowerId: "borrower-james", organizationId: "demo-org",
    status: "CANCELLED", policyNumber: "POL-DEF-345678", policyTypes: ["AUTO"],
    ...
    vehicleRemovedFromPolicy: false, complianceIssues: ["POLICY_CANCELLED", "MISSING_LIENHOLDER", "NO_COMPREHENSIVE", "NO_COLLISION"],
    dashboardStatus: "RED", lastVerifiedAt: Timestamp.now(), createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
  });
  console.log("  ✓ Policy: James Wilson (RED — cancelled, no lienholder)");

  // RED: Sarah Chen — missing lienholder, high deductible
  await db.collection("policies").doc("policy-sarah").set({
    id: "policy-sarah", vehicleId: "vehicle-sarah", borrowerId: "borrower-sarah", organizationId: "demo-org",
    status: "ACTIVE", policyNumber: "POL-GHI-901234", policyTypes: ["AUTO"],
    ...
    vehicleRemovedFromPolicy: false, complianceIssues: ["MISSING_LIENHOLDER", "DEDUCTIBLE_TOO_HIGH"],
    dashboardStatus: "RED", lastVerifiedAt: Timestamp.now(), createdAt: Timestamp.now(), updatedAt: Timestamp.now(),
  });
  console.log("  ✓ Policy: Sarah Chen (RED — missing lienholder, high deductible)");

  // 7. Audit log entry
  await db.collection("auditLog").doc().set({
    organizationId: "demo-org", entityType: "BORROWER", entityId: "demo-borrower",
    action: "CREATED", newValue: { firstName: "John", lastName: "Smith", loanNumber: "LN-2024-001" },
    performedBy: userRecord.uid, timestamp: Timestamp.now(),
  });
  console.log("  ✓ Audit log entry");

  console.log("\n✅ Production seed complete! Refresh the dashboard.");
}
